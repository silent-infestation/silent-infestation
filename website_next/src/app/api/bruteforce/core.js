...
async function detectAntiBruteforceProtection(formUrl, formData, method, formName) {
  ...
  for (let i = 0; i < bruteforceConfig.maxAttempts; i++) {
    ...
    const passwordField =
      Object.keys(testData).find(
        (field) => field.toLowerCase().includes("pass") || field.toLowerCase().includes("pwd")
    ...
    try {
      ...
      if (i > 0) {
        if (
          responseTime > previousResponseTime * 1.5 &&
          responseTime > bruteforceConfig.timeoutThreshold
        ) {
          consecutiveDelays++;
          if (consecutiveDelays >= 2) {
            // Requiert au moins 2 délais consécutifs pour confirmer
      ...
      const isAccountLocked = accountLockedPatterns.some(
        (pattern) =>
          pattern.test(response.data) || pattern.test(response.headers["x-error-message"] || "")
      );
